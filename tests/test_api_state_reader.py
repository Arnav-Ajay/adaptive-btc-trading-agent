...
def test_load_dashboard_state_includes_simulation_artifacts(tmp_path) -> None:
    ...
    (state_dir / "simulation_latest.json").write_text(json.dumps({"candidate_count": 4}), encoding="utf-8")
    (state_dir / "simulation_history.jsonl").write_text(json.dumps({"recorded_at": "2026-03-25T00:00:00+00:00"}) + "\n", encoding="utf-8")
...
def test_load_dashboard_state_can_skip_backtest_and_simulation_artifacts(tmp_path) -> None:
    ...
    (state_dir / "backtest_latest.json").write_text(json.dumps({"interval": "30m"}), encoding="utf-8")
    (state_dir / "backtest_history.jsonl").write_text(json.dumps({"recorded_at": "2026-03-25T00:00:00+00:00"}) + "\n", encoding="utf-8")
    (state_dir / "simulation_latest.json").write_text(json.dumps({"candidate_count": 4}), encoding="utf-8")
    (state_dir / "simulation_history.jsonl").write_text(json.dumps({"recorded_at": "2026-03-25T00:00:00+00:00"}) + "\n", encoding="utf-8")
